...
public class ExtendedSlabs {
	...
	private void gatherData(final GatherDataEvent event) {
		...
		dataGenerator.addProvider(event.includeServer(), new ESPLootTableProvider(dataGenerator));
		dataGenerator.addProvider(event.includeClient(), new ESPBlockModelProvider(dataGenerator, MODID, existing));
		dataGenerator.addProvider(event.includeClient(), new ESPItemModelGenerator(dataGenerator, existing));
		dataGenerator.addProvider(event.includeClient(), new ESPBlockStateProvider(dataGenerator, MODID, existing));
		dataGenerator.addProvider(event.includeServer(), new ESPBlockTagsProvider(dataGenerator, existing));
		dataGenerator.addProvider(event.includeServer(), new ESPStairRecipeProvider(dataGenerator));
